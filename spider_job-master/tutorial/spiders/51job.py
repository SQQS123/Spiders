...
class FiveJobSpider(scrapy.Spider):
    ...
    # 上海PHP
    start_urls = [
        "https://m.51job.com/search/joblist.php?jobarea=170200&keyword=Python&keywordtype=2&pageno=1",
    ]
    custom_settings = {
        "ITEM_PIPELINES":{
    ...
    def parse(self, response):
        ...
        # 下面是Engine从Spider获取第一个要爬取的URL，并通过Scheduler以Request的形式调度。
        # 循环使用parse_item分析这一页的每一个连接的页面内容，并得到item数据
        for url in items:
            # print("开始分析一页上面的所有岗位信息:"+url)
            yield Request(url,callback=self.parse_item)

        # 开始搞下一页
        if self.current_page < self.max_page:
            self.current_page += 1
            nex_page = 'https://m.51job.com/search/joblist.php?jobarea=170200&keyword=Python&keywordtype=2&pageno='+str(self.current_page)
            # 这里的每个页面需要使用parse来分析
